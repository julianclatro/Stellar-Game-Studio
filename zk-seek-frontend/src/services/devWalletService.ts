...
class DevWalletService {
  ...
  /**
   * Get the admin signer for admin-only operations (e.g. resolve_game)
   * Uses VITE_DEV_ADMIN_SECRET from environment
   */
  getAdminSigner(): { address: string; signer: ContractSigner } {
    const adminSecret = import.meta.env.VITE_DEV_ADMIN_SECRET;
    if (!adminSecret) {
      throw new Error('Admin secret not available. Run "bun run setup" first.');
    }
      ...
      signer: {
        signTransaction: async (txXdr: string, opts?: any) => {
          try {
            ...
            return {
              ...
              signerAddress: adminPublicKey,
            };
          } catch (error) {
            console.error('Failed to sign transaction (admin):', error);
            return {
              signedTxXdr: txXdr,
              signerAddress: adminPublicKey,
        ...
        signAuthEntry: async (preimageXdr: string, _opts?: any) => {
          try {
            ...
            const signatureBytes = adminKeypair.sign(payload);
            return {
              signedAuthEntry: Buffer.from(signatureBytes).toString('base64'),
              signerAddress: adminPublicKey,
            };
          } catch (error) {
            console.error('Failed to sign auth entry (admin):', error);
            return {
