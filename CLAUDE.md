...
- `contracts/template/` baseline game contract used by the create script
...
- `frontend-template/` standalone frontend template used by the create script
...
3. Implement contract logic in `contracts/<game-name>/src/lib.rs` using the template pattern.
...
- Use the mock Game Hub pattern from `contracts/template/src/test.rs` or `contracts/mock-game-hub`.
